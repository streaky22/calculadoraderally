...
export const PublicView: React.FC<{ rally: Rally }> = ({ rally }) => {
  const [activeTab, setActiveTab] = useState('Stage Times');
  const [activeStageId, setActiveStageId] = useState<string>(rally.stages[0]?.id || '');

  const tabs = ["Overall", "Stage Times", "Stage Winners", "Start Lists", "Penalties", "Retirements"];
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {(() => {
                  ...
                  return penaltiesList.map(({ st, participant, stage, penalty }, idx) => (
                    <tr key={`${st.id}-${penalty?.id}-${idx}`} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-bold text-gray-900">{participant?.driverName}</td>
                      <td className="px-4 py-3 text-gray-600">{stage?.identifier}</td>
                      <td className="px-4 py-3 text-gray-600">{penalty?.name}</td>
